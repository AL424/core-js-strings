...
function reverseString(str) {
  let result = '';
  for (let i = str.length - 1; i >= 0; i -= 1) result += str.charAt(i);
  return result;
}
...
function countVowels(str) {
  ...
  let result = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (vowels.includes(str.charAt(i))) result += 1;
  }
  return result;
}
...
function isPalindrome(str) {
  const clearStr = str.replaceAll(/\W/g, '').toLowerCase();
...
function encodeToRot13(str) {
  ...
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    const index = alfabet.indexOf(str.charAt(i));
    if (index === -1) result += str.charAt(i);
    else result += rot13.charAt(index);
  }
  return result;
}
